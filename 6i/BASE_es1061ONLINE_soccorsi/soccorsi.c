...
int NumSoccorritoriCheHannoFinito = 0;

pthread_cond_t condLiberaSoccoritori;
pthread_cond_t condWaitTuttiISoccorritori;
...
void *Soccorritore(char *Label, int ruolo)
{
	while (1)
	{
		...
		while (NumSoccorritoriArrivati < NUMESORCISTI + NUMMEDICI)
		{
			DBGpthread_cond_wait(&condWaitTuttiISoccorritori, &mutex, Label);
		}

		/* da completare */
		DBGpthread_cond_broadcast(&condWaitTuttiISoccorritori, "DBGpthread_cond_broadcast failed");
		...
		/* da completare */
		NumSoccorritoriCheHannoFinito++;
		if (NumSoccorritoriCheHannoFinito < NUMMEDICI + NUMESORCISTI)
		{
			DBGpthread_cond_wait(&condLiberaSoccoritori, &mutex, "DBGpthread_cond_wait failed");
		}

		printf("soccorritore %s va via \n", Label);

		DBGpthread_cond_broadcast(&condLiberaSoccoritori, "DBGpthread_cond_broadcast falied");

		richiestaAiuto = 0;
		DBGpthread_cond_signal(&condFineSoccorso, "DBGpthread_cond_signal failed");

		NumSoccorritoriArrivati = 0;
		NumSoccorritoriCheHannoFinito = 0;
...
int main(int argc, char *argv[])
{
	...
	DBGpthread_cond_init(&condLiberaSoccoritori, NULL, "DBGpthread_cond_init failed");
	DBGpthread_cond_init(&condWaitTuttiISoccorritori, NULL, "DBGpthread_cond_init failed");
