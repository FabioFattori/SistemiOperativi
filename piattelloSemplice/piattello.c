...
void *Tiratore(void *arg)
{
	...
	while (1)
	{
		...
		{
			DBGpthread_cond_wait(&condSvegliaTiratori, &mutex, "e allora non aspettare merda");
		}
		...
		*/
		...
		{
			printf("EVVAI PORCODIUO (detto rispettosamente da %s)\n", Plabel);
		...
		DBGpthread_mutex_unlock(&mutex, "esplodui");
	pthread_exit(NULL);
}
...
void *Piattello(void *arg)
{
	...
	DBGpthread_mutex_unlock(&mutex, Plabel);
	DBGpthread_cond_broadcast(&condSvegliaTiratori, "eslopi");
	...
	*/
	...
	DBGpthread_mutex_lock(&mutex, "male male");
	...
	DBGpthread_mutex_unlock(&mutex, "kkk");
